...
function getTurnsScript() {
    ...
    (() => {
      ...
      const messageList = document.querySelector('[data-testid="message-list"], [class*="message-list"], .list_items');
      ...
      const fallbackRows = Array.from(messageList.querySelectorAll('.v_list_row, [class*="v_list_row"]'))
        .filter((el) => isVisible(el));
      const fallbackTurns = [];
      const fallbackSeen = new Set();
      let pendingAssistantText = '';
      for (const row of fallbackRows) {
        const isAssistant = !!row.querySelector('[class*="flow-markdown-body"], [class*="markdown"], [class*="paragraph"]');
        const isUser = !isAssistant && (!!row.querySelector('[class*="justify-end"]')
          || /\bjustify-end\b/.test(String(row.className || '')));
        const role = isUser ? 'User' : (isAssistant ? 'Assistant' : '');
        if (!role) continue;
        const textRoot = isAssistant
          ? row.querySelector('[class*="flow-markdown-body"], [class*="markdown"], [class*="paragraph"]')
          : row;
        const rowText = clean(textRoot?.innerText || textRoot?.textContent || '');
        if (!rowText || rowText.length > 4000) continue;
        if (role === 'Assistant') {
          pendingAssistantText = rowText;
          continue;
        }
        if (pendingAssistantText) {
          const key = 'Assistant::' + pendingAssistantText;
          if (!fallbackSeen.has(key)) {
            fallbackSeen.add(key);
            fallbackTurns.push({ Role: 'Assistant', Text: pendingAssistantText });
          }
          pendingAssistantText = '';
        }
        const key = role + '::' + rowText;
        if (fallbackSeen.has(key)) continue;
        fallbackSeen.add(key);
        fallbackTurns.push({ Role: role, Text: rowText });
      }
      if (pendingAssistantText) {
        const key = 'Assistant::' + pendingAssistantText;
        if (!fallbackSeen.has(key)) {
      ...
      if (fallbackTurns.length > 0) return fallbackTurns;
      return [];
    })()
  `;
}
function getAssistantBusyScript() {
    return `
    (() => {
      ...
      const stopSignals = Array.from(document.querySelectorAll('button, [role="button"]'))
        .filter((node) => isVisible(node))
        .map((node) => [clean(node.innerText || node.textContent || ''), clean(node.getAttribute('aria-label') || ''), clean(node.getAttribute('title') || '')].join(' '))
        .some((text) => /停止|中止|stop|cancel/i.test(text));
      const sendButton = document.querySelector('#flow-end-msg-send, button[id="flow-end-msg-send"]');
      const sendDisabled = sendButton instanceof HTMLElement
        ? sendButton.getAttribute('disabled') !== null || sendButton.getAttribute('aria-disabled') === 'true'
        : false;
      return stopSignals || sendDisabled;
    })()
  `;
}
...
function clickSendButtonScript() {
    ...
    (() => {
      ...
      const directButton = document.querySelector('#flow-end-msg-send, button[id="flow-end-msg-send"]');
      if (directButton instanceof HTMLElement && isVisible(directButton)) {
        const disabled = directButton.getAttribute('disabled') !== null
      ...
      const rootCandidates = [
        ...
        composer.closest('#input-engine-container'),
...
async function selectPreferredDoubaoTab(page) {
    ...
    if (tabs.length === 0)
        return false;
    const conversationTabs = tabs.filter((tab) => /https:\/\/www\.doubao\.com\/chat\/\d+/.test(tab.url));
    if (conversationTabs.length === 0)
        return false;
    const preferred = [...conversationTabs].sort((left, right) => {
        const score = (tab) => {
            let value = tab.index;
            if (/https:\/\/www\.doubao\.com\/chat\/\d+/.test(tab.url))
                value += 1000;
...
async function openMostRecentDoubaoConversation(page) {
    const sidebarConversation = await page.evaluate(`
      (() => {
        const link = Array.from(document.querySelectorAll('a[id^="conversation_"], a[href^="/chat/"]'))
          .find((node) => node instanceof HTMLAnchorElement && /\\/chat\\/[0-9]+/.test(node.getAttribute('href') || ''));
        return link instanceof HTMLAnchorElement ? link.href : '';
      })()
    `).catch(() => '');
    if (!sidebarConversation)
        return false;
    await page.goto(sidebarConversation, { waitUntil: 'load', settleMs: 2000 });
    await page.wait(0.8);
    return true;
}
export async function ensureDoubaoChatPage(page) {
    ...
    if (typeof currentUrl === 'string' && currentUrl.includes('doubao.com/chat')) {
        if (!/https:\/\/www\.doubao\.com\/chat\/\d+/.test(currentUrl)) {
            const selected = await selectPreferredDoubaoTab(page).catch(() => false);
            if (!selected)
                await openMostRecentDoubaoConversation(page).catch(() => false);
        }
        await page.wait(1);
        return;
    }
...
export async function waitForDoubaoResponse(page, beforeLines, beforeTurns, promptText, timeoutSeconds) {
    ...
    const getCandidate = async () => {
        ...
        const promptIndex = [...turns]
            ...
            .reverse()
            .find(({ turn }) => turn.Role === 'User' && (normalizePromptText(turn.Text) === normalizedPromptText
            || normalizePromptText(turn.Text).includes(normalizedPromptText)
            || normalizedPromptText.includes(normalizePromptText(turn.Text))))?.index ?? -1;
        if (turns.length > 0 && promptIndex < 0)
            return '';
        const assistantCandidate = promptIndex >= 0
            ? turns.slice(promptIndex + 1).find((turn) => turn.Role === 'Assistant')
            : [...turns]
                .reverse()
        ...
        const busy = await page.evaluate(getAssistantBusyScript()).catch(() => false);
        if (promptIndex >= 0 && busy)
            return '';
        if (visibleCandidate && !busy)
