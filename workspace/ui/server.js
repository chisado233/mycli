...
const UI_REGISTRY_ROOT = path.join(WORKSPACE_ROOT, 'ui');
...
function normalizeManifest(raw, manifestPath, nodeDir) {
  ...
  return {
    ...
    commands,
    manifestPath,
    registryPath: manifestPath,
    root: raw.root || nodeDir
  };
}

function registryKey(domain, name) {
  const normalized = String(name || '').replace(/\\+/g, '/').replace(/^\/+|\/+$/g, '');
  if (domain === 'mycli') return `mycli:${normalized || 'root'}`;
  if (domain === 'projects') return `project:${normalized}`;
  return `${domain}:${normalized}`;
}

function loadRegistryManifests(domain) {
  const root = path.join(UI_REGISTRY_ROOT, domain);
  const map = new Map();
  function walk(dir) {
    for (const ent of safeReaddir(dir)) {
      ...
      const relativeNoExt = relative.replace(/\.json$/i, '');
      const dirRelative = path.dirname(relative);
      const fallbackPath = dirRelative && dirRelative !== '.' ? dirRelative : relativeNoExt;
      const targetDomain = raw.target && raw.target.domain ? raw.target.domain : domain;
      const targetPath = raw.target && raw.target.path ? raw.target.path : fallbackPath;
      if (targetDomain !== domain || !targetPath) continue;
      const key = registryKey(domain, targetPath);
      map.set(key, { raw, manifestPath: file });
    }
  }
  walk(root);
  return map;
...
function loadUiRegistry() {
  return {
    mycli: loadRegistryManifests('mycli'),
    projects: loadRegistryManifests('projects')
  };
}

function getManifestForNode(fullPath, id, registry) {
  const registered = registry && registry.get(id);
  if (registered) return normalizeManifest(registered.raw, registered.manifestPath, fullPath);
  const manifestPath = findManifest(fullPath);
  return manifestPath ? normalizeManifest(readJson(manifestPath), manifestPath, fullPath) : null;
}

function directoryNode({ id, name, kind, fullPath, relPath, children = [], registry }) {
  const manifest = getManifestForNode(fullPath, id, registry);
  return {
    id,
    name,
    kind,
...
function buildMycliTree() {
  const registry = buildMycliTree.registry || new Map();
  const utilityDirs = new Set(['scripts', 'public', 'state', 'logs', 'tasks', 'config', 'docs', 'tmp', 'ui', 'references', 'agents', 'source']);
  function walk(dir, relParts, depth) {
    ...
    if (depth < 5) {
      for (const ent of safeReaddir(dir)) {
        ...
        const childId = `mycli:${[...relParts, ent.name].join('/')}`;
        const hasManifest = Boolean(findManifest(childDir) || registry.has(childId));
    ...
    return directoryNode({ id: `mycli:${relParts.join('/') || 'root'}`, name, kind: 'mycli-package', fullPath: dir, relPath, children, registry });
  }
  return walk(MYCLI_ROOT, [], 0);
}

function buildProjectsTree() {
  const registry = buildProjectsTree.registry || new Map();
  ...
  return directoryNode({ id: 'projects:root', name: 'projects', kind: 'project-root', fullPath: PROJECTS_ROOT, relPath: 'projects', children, registry });
}

function buildTree() {
  const uiRegistry = loadUiRegistry();
  buildMycliTree.registry = uiRegistry.mycli;
  buildProjectsTree.registry = uiRegistry.projects;
  return directoryNode({
    id: 'workspace:root',
