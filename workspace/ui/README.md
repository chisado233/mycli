...
`workspace ui` 是 `mycli workspace` 的统一 UI 总控台。它以树形目录展示 `capability-library`、`mycli` 包树和 `projects` 项目区，并负责发现、启动、停止、打开各节点自己暴露的子 UI。
...
有子 UI 的包或项目统一登记在 workspace 顶层 `ui` root 下。`ui` 和 `tmp`、`cache`、`logs` 同级，并使用同样的 namespace 文件树：

```text
D:\agent_workspace\ui\mycli\<package-ui>.json
D:\agent_workspace\ui\mycli\<package-path>\ui.json
D:\agent_workspace\ui\projects\<project-ui>.json
D:\agent_workspace\ui\projects\<project-id>\ui.json
D:\agent_workspace\ui\skills\<skill-ui>.json
D:\agent_workspace\ui\agents\<agent-ui>.json
D:\agent_workspace\ui\shared\<shared-ui>.json
```

推荐新接入使用目录树形式，例如 `D:\agent_workspace\ui\mycli\cron\ui.json`、`D:\agent_workspace\ui\mycli\startup\ui.json`、`D:\agent_workspace\ui\mycli\channels\monitor-ui\ui.json`，这样 `ui` root 的结构会和 `tmp/cache/logs/...` 的对象树一致。

每个 JSON 对应一个 mycli 包或项目 UI，必须声明 `target`：
...
为兼容旧接入方式，服务仍会读取节点目录下的 `.agent-ui.json` / `ui.manifest.json`，但新接入统一放入 `D:\agent_workspace\ui\...`。

## Scope

- `capability-library/mycli`：展示包树，并按 `D:\agent_workspace\ui\mycli\*.json` 将 UI 绑定到对应包节点。
- `projects`：展示一级项目目录，并按 `D:\agent_workspace\ui\projects\*.json` 将 UI 绑定到对应项目节点。
...
- 接入 `workspace ui` 的 manifest 统一放在 `D:\agent_workspace\ui\...` 标准树，用 `target.domain` 与 `target.path` 绑定到具体 mycli 包或项目。
